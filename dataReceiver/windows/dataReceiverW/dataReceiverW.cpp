...
//static unsigned int	reqsize = 1;	// Request size in number of packets in Linux 1 in Windows 32 Why??
static unsigned int	reqsize = 32;	// Request size in number of packets
...
static int parseUSBpacket(void* buf, int len, void* bufOut) {
	...
	dataOUT_as_header = (data_header_struct_out*)bufOut;
	dataOUT_as_payload = (data_payload_struct_out*)bufOut;
	uint32_t val;
	...
	bad_packet_count = 0;
	int output_pos = 0;
	dataIN_as_header = (data_header_struct_in*)buf;
	dataIN_as_payload = (data_payload_struct_in*)buf;
	...
	if (len > 0) {
		...
		if (saving_data_flag) {
			...
			//fprintf(stderr,"%s",filename);
				   //fprintf(stderr,"%p  %ld\n ", bufOut, bufOutSize);
			myfile.write((const char*) bufOut, bufOutSize);
			/*if (fwrite(bufOut, 1, bufOutSize, fptr) != bufOutSize) {
	...
	struct libusb_transfer* transfer)
{
	...
	else
	{
		if (saving_data_flag) {
			...
			myfile.write( (char*) (transfer->buffer), transfer->actual_length);
			//fclose(fptr);
		}
		if (streamOut_flag) {
